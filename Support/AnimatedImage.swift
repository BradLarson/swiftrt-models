...
struct AnimatedImage {
  ...
  public init(_ frames: [TensorR3<Float>]) {
  ...
  func quantize(_ frames: [TensorR3<Float>]) -> (quantizedFrames: [[UInt8]], palette: [UInt8]) {
    return (frames[0].shape[2] == 1) ? quantizeGrey(frames) : quantizeColor(frames)
  }

  func quantizeGrey(_ frames: [TensorR3<Float>]) -> (quantizedFrames: [[UInt8]], palette: [UInt8]) {
    let palette: [UInt8] = GIF.greyscale64Palette

    var quantizedFrames: [[UInt8]] = []
    for frame in frames {
      let scalars = frame.flatArray
  ...
  func quantizeColor(_ frames: [TensorR3<Float>]) -> (quantizedFrames: [[UInt8]], palette: [UInt8]) {
...
public extension Array where Element == TensorR3<Float> {
